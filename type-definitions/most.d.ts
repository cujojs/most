...
declare namespace Most {
  ...
  function create<A>(f: (add: Function, end: Function, error: Function) => void): Stream<A>;
  function just<A>(a: A): Stream<A>;
  function of<A>(a: A): Stream<A>;
  function from<A>(as: Iterable<A>): Stream<A>;
  function periodic<A>(period: number, a?: A): Stream<A>;

  function unfold<A, B, S>(f: (seed: S) => SeedValue<S, B|Promise<B>>, seed: S): Stream<B>;
  function iterate<A>(f: (a: A) => A|Promise<A>, a: A): Stream<A>;
  function generate<A>(g: CreateGenerator<A>, ...args: Array<any>): Stream<A>;

  function reduce<A, B>(f: (b: B, a: A) => B, b: B, s: Stream<A>): Promise<B>;
  function observe<A>(f: (a: A) => any, s: Stream<A>): Promise<any>;
  function forEach<A>(f: (a: A) => any, s: Stream<A>): Promise<any>;
  function drain<A>(s: Stream<A>): Promise<any>;

  function constant<A, B>(b: B, s: Stream<A>): Stream<B>;
  function map<A, B>(f: (a: A) => B, s: Stream<A>): Stream<B>;
  function tap<A>(f: (a: A) => any, s: Stream<A>): Stream<A>;
  function ap<A, B>(fs: Stream<(a: A) => B>, as: Stream<A> ): Stream<B>;
  function chain<A, B>(f: (a: A) => Stream<B>, s: Stream<A>): Stream<B>;
  function flatMap<A, B>(f: (a: A) => Stream<B>, s: Stream<A>): Stream<B>;
  function join<A>(s: Stream<Stream<A>>): Stream<A>;
  function switchLatest<A>(s: Stream<Stream<A>>): Stream<A>;

  function continueWith<A>(f: (a: any) => Stream<A>, s: Stream<A>): Stream<A>;
  function concatMap<A, B>(f: (a: A) => Stream<B>, s: Stream<A>): Stream<B>;
  function mergeConcurrently<A>(concurrency: number, s: Stream<Stream<A>>): Stream<A>;
  function merge<A>(...ss: Array<Stream<A>>): Stream<A>;
  function combine<A>(f: (...args: Array<any>) => A, ...ss: Array<Stream<any>>): Stream<A>;

  function scan<A, B>(f: (b: B, a: A) => B, b: B, s: Stream<A>): Stream<B>;
  function loop<A, B, S>(f: (seed: S, a: A) => SeedValue<S, B>, seed: S, s: Stream<A>): Stream<B>;

  function cycle<A>(s: Stream<A>): Stream<A>;
  function concat<A>(s1: Stream<A>, s2: Stream<A>): Stream<A>;
  function startWith<A>(a: A, s: Stream<A>): Stream<A>;

  function filter<A>(p: (a: A) => boolean, s: Stream<A>): Stream<A>;
  function skipRepeats<A>(s: Stream<A>): Stream<A>;
  function skipRepeatsWith<A>(eq: (a1: A, a2: A) => boolean, s: Stream<A>): Stream<A>;

  function take<A>(n: number, s: Stream<A>): Stream<A>;
  function skip<A>(n: number, s: Stream<A>): Stream<A>;
  function takeWhile<A>(p: (a:  A) => boolean, s: Stream<A>): Stream<A>;
  function skipWhile<A>(p: (a: A) => boolean, s: Stream<A>): Stream<A>;
  function slice<A>(start: number, end: number, s: Stream<A>): Stream<A>;

  function until<A>(signal: Stream<any>, s: Stream<A>): Stream<A>;
  function takeUntil<A>(signal: Stream<any>, s: Stream<A>): Stream<A>;
  function since<A>(signal: Stream<any>, s: Stream<A>): Stream<A>;
  function skipUntil<A>(signal: Stream<any>, s: Stream<A>): Stream<A>;
  function during<A>(timeWindow: Stream<Stream<any>>, s: Stream<A>): Stream<A>;
  function throttle<A>(period: number, s: Stream<A>): Stream<A>;
  function debounce<A>(period: number, s: Stream<A>): Stream<A>;

  function timestamp<A>(s: Stream<A>): Stream<TimeValue<A>>;
  function delay<A>(dt: number, s: Stream<A>): Stream<A>;

  function fromPromise<A>(p: Promise<A>): Stream<A>;
  function awaitPromises<A>(s: Stream<Promise<A>>): Stream<A>;

  function sample<A>(f: (...as: any[]) => A, sampler: Stream<any>, ...ss: Array<Stream<any>>): Stream<A>;
  function sampleWith<A>(sampler: Stream<any>, s: Stream<A>): Stream<A>;

  function zip<A>(f: (...as: any[]) => A, ...ss: Array<Stream<any>>): Stream<A>;

  function recoverWith<A, B>(p: (a: B) => A, s: Stream<A>): Stream<A>;
  function throwError(e: Error): Stream<any>;

  function multicast<A>(s: Stream<A>): Stream<A>;
}
