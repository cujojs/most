...
var suite = Benchmark.Suite('flatMap(empty or just(x)) ' + n + ' integers');
var options = {
	defer: true,
	onError: function(e) {
...
suite
	.add('most', function(deferred) {
		// runners.runMost(deferred, most.from(a).chain(most.just).reduce(sum, 0));
		runners.runMost(deferred, most.from(a).chain(function(x) { return x % 2 === 0 ? most.just(x) : most.empty(); }).reduce(sum, 0));
	}, options)
	// .add('rx 4', function(deferred) {
	// 	runners.runRx(deferred, rx.Observable.fromArray(a).flatMap(rx.Observable.of).reduce(sum, 0));
	// }, options)
	.add('rx 5', function(deferred) {
		runners.runRx5(deferred,
			rxjs.Observable.from(a).flatMap(function(x) { return x % 2 === 0 ? rxjs.Observable.of(x) : rxjs.Observable.empty(); }).reduce(sum, 0));
	}, options)
