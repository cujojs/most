...
Join.prototype.run = function(sink, scheduler) {
	return new OuterJoinSink(this.f, this.source, sink, scheduler);
};

function OuterJoinSink(f, source, sink, scheduler) {
	this.f = f;
	this.sink = sink;
...
InnerJoinSink.prototype.error = function(t, e) {
	this.outer.error(Math.max(t, this.startTime), e);
};
