...
function scan(f, initial, stream) {
	return cons(initial, new Stream(scanSource(f, initial, stream.source)));
}

function scanSource(f, initial, source) {
	var g = source instanceof Map ? compose(source.f, f) : f;
	return new Accumulate(ScanSink, g, initial, source)
}

function compose(map, accum) {
	return function(c, a) {
		return accum(c, map(a));
	};
}
