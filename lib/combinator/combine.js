...
function combine(f /*, ...streams */) {
	return new Stream(new Combine(f, map(flatten(rest(arguments)), getSource)));
}
...
function combineArray(f, streams) {
	return streams.length === 0 ? core.empty()
		 ...
		 : new Stream(new Combine(f, map(streams, getSource)));
}
...
CombineSink.prototype.event = function(t /*, indexSink */) {
	...
	if(this.ready) {
		...
		this.sink.event(t, invoke(this.f, map(this.sinks, getValue)));
	}
};
