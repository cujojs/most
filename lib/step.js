...
var reduce = require('./base').reduce;
...
var MAX = { index: -1, value: { time: Infinity }};
...
function unamb(f, steps) {
	...
	return race(steps).then(function() {
		var winner = reduce(function(winner, p, i) {
			var h = getStatus(p);
			if(h.state() > 0 && h.value.time < winner.value.time) {
				return { index: i, value: h.value };
			}
