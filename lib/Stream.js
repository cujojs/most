...
Stream.from = function(iterable) {
	return isArrayLike(iterable) ? fromArrayLike(iterable)
...
function fromArrayLike(a) {
	return new Stream(stepArrayLike, new Pair(0, a));
}

function stepArrayLike(state) {
	var i = state.value;
	var a = state.state;
	return i < a.length ? new Yield(0, a[i], new Pair(i + 1, a)) : new End(0);
}
