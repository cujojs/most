...
function reduce(f, z, stream) {
	return asap(reduceStep, f, z, stream.step, stream.state, stream.dispose);
}
...
function reduceStep(f, z, stepper, state, dispose) {
	return when(function (i) {
		if(i.done) {
			return when(function() {
				return z;
			}, dispose(i.time, i.value, i.state));
		}
		return i.done ? z
			: reduceStep(f, f(z, i.value), stepper, i.state, dispose);
	}, when(stepper, state));
}