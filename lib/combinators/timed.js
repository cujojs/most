...
function stepDebounce(scheduler, period, step, s) {
	return stepD(scheduler, period, step, s.timer === void 0 ? initState(step, s) : s);
}

function stepD(scheduler, period, step, s) {
	return raceIndex(function(winner, index) {
		return index === 0 ? yieldDebounced(s)
			: winner.done ? winner
			: stepD(scheduler, period, step, nextState(scheduler, period, step, winner));
	}, [s.timer, s.next]);
}
...
function nextState (scheduler, period, step, winner) {
	return { timer: delayed(period, 'timer', scheduler), prev: winner, next: when(step, winner.state), state: winner.state };
}

function yieldDebounced(s) {
	return when(function (prev) {
		return new Yield(prev.value, { timer: never(), next: s.next, prev: void 0, state: prev.state });
	}, s.prev);
}