...
function throttleOn(scheduler, period, stream) {
	return stream.begetWithDispose(function(s) {
		return throttleNext(stream.step, s, period, scheduler);
	}, new Pair(-1, stream.state), function(t, x, s) {
...
function throttleNext(stepper, s, period, scheduler) {
	return when(function(i) {
		if(i.done) {
			return i.withState(new Pair(s.value, i.state));
		}

		return i.time > s.value ? i.withState(new Pair(i.time + period, i.state))
