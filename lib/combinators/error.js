...
function flatMapError(f, stream) {
	return stream.beget(stepFlatMapError, { stream: stream, recover: f }, dispose);
}

function stepFlatMapError (s) {
	return resolve(s.stream.state).then(function (state) {
		return stepWithRecovery(s.recover, s.stream.beget(s.stream.step, state));
	}).catch(function (e) {
		// TODO: Should we call s.stream.dispose here?
		return stepWithRecovery(thrower, s.recover(e));
	});
}

function stepWithRecovery(f, stream) {
	return when(function (i) {
		return i.done ? i
			: i.withState({ stream: stream.beget(stream.step, i.state), recover: f });
