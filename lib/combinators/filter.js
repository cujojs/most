...
function takeUntil(signal, stream) {
	return stream.begetWithDispose(stepTakeUntil,
		new TakeUntil(signal, stream, stream.state), disposeTakeUntil);
}

function stepTakeUntil(s) {
	return unamb(function (i, index) {
		return index === 0 ? endTakeUntil(s, i)
			: i.withState(new TakeUntil(s.signal, s.stream, i.state));
...
function endTakeUntil(s, i) {
	return new End(i.time, i.value, new TakeUntil(null, s.stream, s.state));
}

function TakeUntil(signal, stream, state) {
	this.signal = signal; this.stream = stream; this.state = state;
}
