...
function takeWhile(p, stream) {
	return stream.begetWithDispose(function(s) {
		return stepTakeWhile(p, stream.step, s);
	}, new Pair(void 0, stream.state), function(t, x, s) {
		return stream.dispose(t, x, s.state);
	});
}
...
function stepTakeWhile (p, stepper, s) {
	return when(function (i) {
		return i.done || p(i.value) ? i.withState(new Pair(i.value, i.state))
			: new End(i.time, s.value, new Pair(i.value, i.state));
...
function distinctBy(equals, stream) {
	return stream.begetWithDispose(function(s) {
		return stepDistinct(equals, stream.step, s);
	}, new Pair(init, stream.state), function(t, x, s) {
		return stream.dispose(t, x, s.state);
	});
}
...
function stepDistinct(equals, stepper, s) {
	return when(function(i) {
		...
		if(i.done || s.value === init || !equals(s.value, i.value)) {
			return i.withState(new Pair(i.value, i.state));
		}
