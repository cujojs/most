...
function initTakeUntil (signal, stream) {
	return new TakeUntil(Infinity, signal, stream, stream.state);
}

function updateTakeUntilState(s, newState) {
	return new TakeUntil(s.time, s.signal, s.stream, newState);
}

function endTakeUntil(s, i) {
	return new End(i.time, i.value, new TakeUntil(i.time, null, s.stream, s.state));
}

function TakeUntil(time, signal, stream, state) {
	this.time = time; this.signal = signal;
