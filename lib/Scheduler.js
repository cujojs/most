...
function Task(at, period, run, error, x, y, scheduler) {
	this.time = at;
	this.period = period;
	this.active = true;
	this._run = run;
...
Scheduler.prototype.asap = function(run, error, x, y) {
	var task = new Task(0, -1, run, error, x, y, this);
	return Promise.resolve(task).then(runTask);
};

Scheduler.prototype.delay = function(delay, run, error, x, y) {
	return this.schedule(delay, -1, run, error, x, y);
};
...
Scheduler.prototype.schedule = function(delay, period, run, error, x, y) {
	var now = this.now();
	var task = new Task(now + Math.max(0, delay), period, run, error, x, y, this);
...
Scheduler.prototype.cancelAll = function(f) {
	this._tasks = base.reduce(function(tasks, task) {
		if(!f(task)) {
			tasks.push(task);
		}
