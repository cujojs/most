/** @license MIT License (c) copyright 2010-2014 original author or authors */
/** @author Brian Cavalier */
/** @author John Hann */

var base = require('./base');
var resolve = require('./Promise').resolve;

var findIndex = base.findIndex;
var slice = base.slice;

// Default timer functions
function defaultSetTimer(f, ms) {
	return setTimeout(f, ms);
}

function defaultClearTimer(t) {
	return clearTimeout(t);
}

function Task(time, period, run, x, y) {
	this.time = time;
	this.period = period;
	this.active = true;
	this._run = run;
	this._x = x;
	this._y = y;
}

Task.prototype.run = function() {
	return this._run(this._x, this._y, this.time);
};

function Scheduler(setTimer, clearTimer, now, errorHandler) {
	this.now = now || Date.now;
	this._setTimer = setTimer || defaultSetTimer;
	this._clearTimer = clearTimer || defaultClearTimer;
	this._handleError = errorHandler || logError;
	this._timer = null;
	this._nextArrival = 0;
	this._tasks = [];

	var self = this;
	this._runReadyTasksBound = function() {
		self._runReadyTasks();
	};
}

Scheduler.prototype.delay = function(delay, run, x, y) {
	return this.schedule(delay, 0, run, x, y);
};

Scheduler.prototype.periodic = function(period, run, x, y) {
	return this.schedule(0, period, run, x, y);
};

Scheduler.prototype.schedule = function(delay, period, run, x, y) {
	var now = this.now();
	var task = new Task(now + Math.max(0, delay), Math.max(0, period), run, x, y);

	insertByTime(this._tasks, task);
	this._scheduleNextRun(now);
	return task;
};

Scheduler.prototype.cancel = function(task) {
	task.active = false;
	var i = findIndex(task, this._tasks);
	if(i >= 0) {
		this._tasks.splice(i, 1);

		if(this._tasks.length === 0) {
			this._clearTimer(this._timer);
		} else if(i === 0) {
			this._scheduleNextRun(this.now());
		}
	}
};

Scheduler.prototype._runReadyTasks = function() {
	this._timer = null;

	var now = this.now();
	var tasks = this._tasks;
	var reschedule = [];

	var task;

	while(tasks.length > 0 && tasks[0].time <= now) {
		task = tasks.shift();
		if(task.active) {
			this._runTask(task);

			if(task.period > 0 && task.active) {
				reschedule.push(task);
			}
		}
	}

	this._reschedule(reschedule);
	this._scheduleNextRun(this.now());
};

Scheduler.prototype._reschedule = function(tasks) {
	for(var task, i=0; i<tasks.length; ++i) {
		task = tasks[i];
		if(task.active) {
			task.time = task.time + task.period;
			insertByTime(this._tasks, task);
		}
	}
};

Scheduler.prototype._runTask = function(task) {
	task.run();
};

Scheduler.prototype._scheduleNextRun = function(now) {
	if(this._tasks.length === 0) {
		return;
	}

	var nextArrival = this._tasks[0].time;

	if(this._timer === null) {
		this._schedulerNextArrival(nextArrival, now);
	} else if(nextArrival < this._nextArrival) {
		this._clearTimer(this._timer);
		this._schedulerNextArrival(nextArrival, now);
	}
};

Scheduler.prototype._schedulerNextArrival = function(nextArrival, now) {
	this._nextArrival = nextArrival;
	var delay = Math.max(0, nextArrival - now);
	this._timer = this._setTimer(this._runReadyTasksBound, delay);
};

function insertByTime(tasks, task) {
	tasks.splice(findInsertion(task, tasks), 0, task);
	return tasks;
}

function logError (e) {
	setTimeout(function() {
		throw e;
	}, 0);
}

function findInsertion(x, sortedArray) {
	var lo = 0;
	var hi = sortedArray.length;
	var mid, y;

	while (lo < hi) {
		mid = Math.floor((lo + hi) / 2);
		y = sortedArray[mid];

		if (x.time === y.time) {
			return mid;
		} else if (x.time < y.time) {
			hi = mid;
		} else {
			lo = mid + 1;
		}
	}
	return hi;
}

module.exports = new Scheduler();
